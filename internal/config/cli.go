...
func declareFlags(o *happydns.Options) {
	...
	flag.Var(proxyList(&o.TrustedProxies), "trusted-proxy", "Exact IP address (or CIDR block) of a reverse proxy allowed to set X-Forwarded-For/X-Real-IP; may be repeated or comma separated; use \"none\" to clear what a config file or the environment already set; empty (default) trusts no proxy")
	flag.Var(targetList(&o.OutboundAllowedTargets, "outbound target"), "outbound-allowed-target", "IP address (or CIDR block) that requests made on a user's behalf (provider endpoints, certificate probes, MTA-STS, notification webhooks) may reach even though it is not publicly routable; may be repeated or comma separated; use \"none\" to clear what a config file or the environment already set; empty (default) allows public addresses only (see docs/outbound-targets.md)")
	flag.Var(targetList(&o.ResolverAllowedTargets, "resolver target"), "resolver-allowed-target", "IP address (or CIDR block) that the DNS server chosen in the resolver tool may point at even though it is not publicly routable; does not affect -default-ns nor the local resolver; same syntax as -outbound-allowed-target (see docs/outbound-targets.md)")
	flag.Var(faviconSourceList(&o.FaviconSources), "favicon-source", "Where to get the icons shown next to domains and providers, in order of preference: \"direct\" asks each site itself, \"duckduckgo\" and \"google\" ask that service (faster and lighter, but they only know the sites they crawled and they learn which domains you look up), or any URL template containing {domain}; may be repeated or comma separated; use \"none\" to fetch no icon at all (see docs/favicons.md)")
	...
	flag.StringVar(&o.ServiceHostingHost, "service-hosting-host", o.ServiceHostingHost, "Public FQDN serving the content of the domain services hosted by happyDomain: mail-client auto-configuration and MTA-STS policies (defaults to externalurl host)")
	flag.StringVar(&o.MailAutoconfigHost, "mail-autoconfig-host", o.MailAutoconfigHost, "Deprecated: use -service-hosting-host instead")
