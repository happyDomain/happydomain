...
func declarePluginsRoutes(router *gin.RouterGroup, dep Dependencies, store storage.Storage) {
	apiPluginsRoutes := router.Group("/plugins")
	declareTestPluginsRoutes(apiPluginsRoutes, dep)
}

func declareTestPluginsRoutes(router *gin.RouterGroup, dep Dependencies) {
	tpc := controller.NewTestPluginController(dep.TestPlugin)
