...
func NewAdmin(app *App) *Admin {
	...
	// middleware attached explicitly: it exposes the same business intelligence
	// (entity counts, provider mix, latency profiles) the rest of the admin
	// surface is gated on. The middleware is a passthrough when no password is
	// configured, so unix-socket Prometheus scrapes keep working unchanged.
	router.GET("/metrics", adminmw.AdminAuth(app.cfg), gin.WrapH(promhttp.Handler()))
