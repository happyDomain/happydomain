// This file is part of the happyDomain (R) project.
// Copyright (c) 2020-2025 happyDomain
// Authors: Pierre-Olivier Mercier, et al.
//
// This program is offered under a commercial and under the AGPL license.
// For commercial licensing, contact us at <contact@happydomain.org>.
//
// For AGPL licensing:
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.

package user

import (
	"fmt"
	"time"

	"git.happydns.org/happyDomain/model"
)

type CreateUser struct {
	store      UserStorage
	newsletter happydns.NewsletterSubscriptor
}

func NewCreateUser(store UserStorage, newsletter happydns.NewsletterSubscriptor) *CreateUser {
	return &CreateUser{store, newsletter}
}

func (uc *CreateUser) Create(uinfo happydns.UserInfo) (*happydns.User, error) {
	if uinfo.GetEmail() == "" {
		return nil, fmt.Errorf("user email is required")
	}

	user := &happydns.User{
		Id:        uinfo.GetUserId(),
		Email:     uinfo.GetEmail(),
		CreatedAt: time.Now(),
		LastSeen:  time.Now(),
		Settings:  *happydns.DefaultUserSettings(),
	}

	if err := uc.store.CreateOrUpdateUser(user); err != nil {
		return user, err
	}

	if uinfo.JoinNewsletter() {
		if err := uc.newsletter.SubscribeToNewsletter(uinfo); err != nil {
			return user, fmt.Errorf("newsletter subscription failed: %w", err)
		}
	}

	return user, nil
}
