...
func (ru *resolverUsecase) ResolveQuestion(request happydns.ResolverRequest) (*dns.Msg, error) {
	...
	if err != nil {
		return nil, happydns.ValidationError{err.Error()}
	}
	...
	if r == nil {
		...
		return nil, happydns.ValidationError{"DNS request mal formated."}
		...
		return nil, happydns.NotFoundError{fmt.Sprintf("The domain %q was not found.", request.DomainName)}
		...
		return nil, happydns.ForbiddenError{"Resolver refused to treat our request."}
