...
func (m *mockCheckerUsecase) GetCheckerResponse(checker happydns.Checker) happydns.CheckerResponse {
...
func (m *mockCheckerUsecase) ListCheckerResponses() (map[string]happydns.CheckerResponse, error) {
	res := make(map[string]happydns.CheckerResponse, len(m.checkers))
	for name, checker := range m.checkers {
		res[name] = m.GetCheckerResponse(checker)
	}
	return res, nil
}

func (m *mockCheckerUsecase) ValidateCheckerOptions(_ string, _ happydns.CheckerOptions) error {
	return nil
}
