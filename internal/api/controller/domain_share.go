...
//	@Schemes
//	@Description	List the users the given Domain has been shared with, and the users holding a grant on its Provider. Owner-only.
//	@Tags			domains
//	@Accept			json
//	@Produce		json
//	@Param			domainId	path	string	true	"Domain identifier"
//	@Security		securitydefinitions.basic
//	@Success		200	{object}	happydns.DomainShareStatus
...
func (dc *DomainController) GetDomainShares(c *gin.Context) {
	...
	status, err := dc.domainService.GetDomainShareStatus(user, domain.Id)
	if err != nil {
...
func (dc *DomainController) ShareDomain(c *gin.Context) {
	...
	share, err := dc.domainService.ShareDomain(c.Request.Context(), user, domain.Id, input.User, input.WithProvider)
	if err != nil {
		middleware.ErrorResponse(c, http.StatusInternalServerError, err)
		return
	}
...
func (dc *DomainController) DelDomainShare(c *gin.Context) {
	...
	if err := dc.domainService.UnshareDomain(c.Request.Context(), user, domain.Id, granteeID); err != nil {
		middleware.ErrorResponse(c, http.StatusInternalServerError, err)
		return
	}
