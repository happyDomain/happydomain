...
func (fc *FaviconController) GetDomainFavicon(c *gin.Context) {
	...
	// These bytes come from a host the caller named but are served from our
	// own origin. The service only ever returns an image content type.
	middleware.ServeSandboxedImage(c, contentType, iconBytes)
}
