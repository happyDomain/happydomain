    ...
    {:else}
        <ListGroup flush>
            {#if importableDomainsList.length == 0}
                ...
                {/if}
            {:else}
                {#each filteredImportableDomainsList.map( (dn) => ({ domain: dn, id_provider: provider._id }), ) as domain}
                    <ListGroupItem
                ...
                {#if filteredImportableDomainsList.length != importableDomainsList.length}
                    ...
                    >
                        {$t("domains.and-more-filtered", {
                            count:
                                importableDomainsList.length - filteredImportableDomainsList.length,
                        })}
                    </ListGroupItem>
                {/if}
            {/if}
