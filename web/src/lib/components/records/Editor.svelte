...
<script lang="ts">
    ...
    let classStr = $state(String(record.Hdr.Class));
    let rrtypeStr = $state(String(record.Hdr.Rrtype));

    $effect(() => {
        const parsed = parseInt(classStr, 10);
        if (!isNaN(parsed) && record.Hdr.Class !== parsed) {
            record.Hdr.Class = parsed;
        }
    });

    $effect(() => {
        const parsed = parseInt(rrtypeStr, 10);
        if (!isNaN(parsed) && record.Hdr.Rrtype !== parsed) {
            record.Hdr.Rrtype = parsed;
        }
    });
