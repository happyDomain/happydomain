...
function dsIssues(records: DS[]): ComplianceIssue[] {
    ...
    records.forEach((r, idx) => {
        ...
        const check = checkDigest(digest, r.DigestType, DIGEST_LENGTHS);
        if (check.status === "unknown-type") {
            ...
            // A digest that is not hexadecimal is as unusable as a truncated
            // one, and the same message points at the field to fix.
            if (check.status !== "ok") {
                issues.push({
                    id: "ns.ds-digest-length",
                    severity: "error",
                    params: { expected: check.expected, got: check.got },
                    field,
                    docUrl: RFC4034,
                });
            }
