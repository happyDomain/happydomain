 ...
 * it. Kept byte-for-byte in step with (*abstract.MTASTS).PolicyFile, including
 * its refusal to serve a testing/enforce policy with no authorized MX: that
 * would tell senders no MX is authorized, locking mail out of the domain, so
 * the backend serves nothing rather than that. Returns "" in that case, same
 * as PolicyFile returning nil.
 */
export function renderPolicy(value: AbstractMTASTSBody): string {
    ...
    const mxLines: string[] = [];
    for (const raw of value.mx ?? []) {
        const mx = raw.trim();
        if (!mx) continue;
        mxLines.push("mx: " + mx.replace(/\.$/, ""));
    }

    if (mode !== "none" && mxLines.length === 0) return "";

    const lines = ["version: STSv1", "mode: " + mode, ...mxLines, "max_age: " + maxAge];
