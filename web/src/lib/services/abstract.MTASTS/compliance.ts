...
function mtaStsHostedSync(raw: Record<string, any>, ctx: ComplianceContext): ComplianceIssue[] {
    ...
    if (!hostingEnabled(raw)) {
        ...
        const name: string = raw.policyCNAME?.Hdr?.Name ?? "";
        if (!/^mta-sts(\.|$)/i.test(name)) {
            issues.push({
...
async function mtaStsHostedAsync(
    ...
    // normalized strings rather than matched against each other.
    ...
    const sameMx = wantMx.length === servedMx.length && wantMx.every((mx) => servedMx.includes(mx));
    if (!sameMx) {
        issues.push({
