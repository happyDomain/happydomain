...
<script lang="ts">
    ...
    import type { AbstractGitlabPageVerifBody } from "$lib/services_bodies";
    ...
    interface Props {
        ...
        origin: Domain;
        readonly?: boolean;
        value: AbstractGitlabPageVerifBody;
    }

    let { dn, origin, readonly = false, value = $bindable() }: Props = $props();
    const type = "abstract.GitlabPageVerif";

    // Initialize TXT record if needed
    if (!value["txt"]) {
        const txtRecord = newRR("", getRrtype("TXT")) as dnsTypeTXT;
        txtRecord.Txt = "gitlab-pages-verification-code=";
        value["txt"] = txtRecord;
    }
