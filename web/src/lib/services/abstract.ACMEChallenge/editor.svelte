...
<script lang="ts">
    ...
    import type { AbstractACMEChallengeBody } from "$lib/services_bodies";
    ...
    interface Props {
        ...
        origin: Domain;
        readonly?: boolean;
        value: AbstractACMEChallengeBody;
    }

    let { dn, origin, readonly = false, value = $bindable() }: Props = $props();
    const type = "abstract.ACMEChallenge";

    // Initialize TXT record if needed
    if (!value["txt"]) {
        const txtRecord = newRR("_acme-challenge", getRrtype("TXT")) as dnsTypeTXT;
        txtRecord.Txt = "";
        value["txt"] = txtRecord;
    }

    // ACME challenge uses the TXT record content as-is
    let challengeToken = $state(value["txt"]?.Txt || "");

    // Sync challenge token back to TXT record
    $effect(() => {
        if (value["txt"]) {
            value["txt"].Hdr.Name = "_acme-challenge";
