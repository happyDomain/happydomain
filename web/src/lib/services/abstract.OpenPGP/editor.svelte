...
<script lang="ts">
    ...
    import type { AbstractOpenPGPBody } from "$lib/services_bodies";
    import { domainJoin } from "$lib/dns";
    import type { dnsTypeOPENPGPKEY } from "$lib/dns_rr";
    ...
    $effect(() => {
        const hdr = value["openpgpkey"]?.Hdr;
        if (!hdr) return;

        if (hasher.hash) {
            hdr.Name = domainJoin(hasher.hash, "_openpgpkey", dn);
