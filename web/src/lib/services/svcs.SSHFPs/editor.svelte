...
<script lang="ts">
    ...
    function importKeys(keys: HappydnsSshHostKey[]): number {
        ...
        for (const key of keys) {
            ...
            for (const [type, fingerprint] of [
                [2, key.sha256],
                [1, key.sha1],
            ] as const) {
                const value = (fingerprint || "").toLowerCase();
                if (!value || alreadyPublished(algorithm, type, value)) continue;

                fingerprints().push({ algorithm, type, fingerprint: value });
                added++;
            }
        }
    ...
    /**
     * Turns a backend status into a sentence. Unknown statuses echo back the
     * key, so the message the server sent is used instead of showing it raw.
     */
    function statusMessage(status: string | undefined, errorMsg: string | undefined): string {
        const key = `sshfp.retrieve.status.${status}`;
