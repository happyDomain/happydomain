...
function sshfpSync(raw: Record<string, any>, ctx: ComplianceContext): ComplianceIssue[] {
    ...
    records.forEach((r, idx) => {
        ...
        const expected = FINGERPRINT_LENGTHS[r.type];
        ...
        } else if (!HEX_RE.test(fingerprint)) {
            issues.push({
                id: "sshfp.invalid-fingerprint",
                severity: "error",
                field,
            });
        } else if (fingerprint.length !== expected) {
            issues.push({
                id: "sshfp.fingerprint-length",
                severity: "error",
                params: { hash: FINGERPRINT_NAMES[r.type], expected, got: fingerprint.length },
                field,
            });
        }
