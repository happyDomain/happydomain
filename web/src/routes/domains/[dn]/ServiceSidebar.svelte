...
<ul style:padding-left={servicesPadding}>
    {#each services as service}
        <li style:list-style={service._id === serviceid ? "disclosure-closed" : null}>
            <a
                href={serviceLink(service)}
                class="d-block text-truncate text-body text-decoration-none"
                class:fw-bold={service._id === serviceid}
            ...
            >
                ...
                {/if}
                {#if service._comment}
                    <span class="fst-italic text-muted">{service._comment}</span>
                {/if}
            </a>
        </li>
    {/each}
