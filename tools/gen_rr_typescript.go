...
func main() {
	...
	fmt.Fprintln(fd, "// The rdata of an SVCB key or an EDNS0 option depends on the code carrying")
	fmt.Fprintln(fd, "// it, which the record alone knows: both stay opaque here.")
	fmt.Fprintln(fd, "export type SVCBKeyValue = unknown;\n")
	fmt.Fprintln(fd, "export type EDNS0 = unknown;\n")
