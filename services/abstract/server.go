...
next_pool:
	for dn, rrs := range pool {
		...
		for _, rr := range rrs {
			if s.A != nil {
				s.A = utils.RRRelative(s.A, a.GetOrigin()).(*dns.A)
			}
			if s.AAAA != nil {
				s.AAAA = utils.RRRelative(s.AAAA, a.GetOrigin()).(*dns.AAAA)
			}
			for i := range s.SSHFP {
				s.SSHFP[i] = utils.RRRelative(s.SSHFP[i], a.GetOrigin()).(*dns.SSHFP)
			}
