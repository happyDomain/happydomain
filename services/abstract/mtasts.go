...
//
// It returns nil when the service carries no policy at all, which is how a
// caller distinguishes "nothing to serve" from "an empty policy".
func (s *MTASTS) PolicyFile() []byte {
	if s.Mode == "" && len(s.MX) == 0 && s.MaxAge == 0 {
		return nil
	}
...
func mtasts_analyze(a *svc.Analyzer) error {
	for _, record := range a.SearchRR(svc.AnalyzerRecordFilter{Type: dns.TypeTXT, Prefix: "_mta-sts."}) {
		...
		for _, candidate := range a.SearchRR(svc.AnalyzerRecordFilter{Type: dns.TypeCNAME, Prefix: "mta-sts." + domain}) {
			if cname, ok := candidate.(*dns.CNAME); ok && cname.Header().Name == "mta-sts."+domain {
				policyCNAME = cname
				break
			}
		}
