...
// Blank/whitespace-only MX entries must not count towards satisfying the
// "at least one mx" guard: serving enforce/testing with zero actual mx lines
// would tell senders no MX is authorized, locking mail out of the domain.
func TestMTASTS_PolicyFile_BlankMXOnly(t *testing.T) {
	s := &abstract.MTASTS{
		Mode:        "enforce",
		MaxAge:      604800,
		MX:          []string{"", "  "},
	...
	if body := s.PolicyFile(); body != nil {
		t.Errorf("PolicyFile() = %q; want nil (no non-blank mx)", body)
	}
}

// Leaving the hosting checkbox off means self-hosting: happyDomain must not
// serve a policy file it was never asked to serve, even if Mode/MX/MaxAge are
// otherwise fully configured.
func TestMTASTS_PolicyFile_NotHosted(t *testing.T) {
	s := &abstract.MTASTS{
		Mode:   "enforce",
		MaxAge: 604800,
		MX:     []string{"mail.example.com"},
	...
	if body := s.PolicyFile(); body != nil {
		t.Errorf("PolicyFile() = %q; want nil (no PolicyCNAME)", body)
	}
}
...
func TestMTASTS_Analyze_WithPolicyHost(t *testing.T) {
	...
	defer abstract.SetServiceHostingHost("")
...
// A CNAME using the same _mta-sts./mta-sts. naming convention but pointing at
// a third-party host must not be misclassified as hosted by happyDomain.
func TestMTASTS_Analyze_WithPolicyHost_TargetMismatch(t *testing.T) {
	abstract.SetServiceHostingHost("happydomain.example.com")
	defer abstract.SetServiceHostingHost("")
	...
	cname, err := dns.NewRR(`mta-sts.example.com. 3600 IN CNAME third-party.example.net.`)
	if err != nil {
		t.Fatalf("dns.NewRR: %v", err)
	}
	...
	for _, s := range services[""] {
		if _, ok := s.Service.(*abstract.MTASTS); ok {
			t.Fatalf("mismatched CNAME target must not be claimed as abstract.MTASTS")
		}
	}
}
