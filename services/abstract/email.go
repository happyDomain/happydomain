...
func (s *EMail) GetRecords(domain string, ttl uint32, origin string) (rrs []happydns.Record, e error) {
	if len(s.MX) > 0 {
		mx_rrs, err := (&svcs.MXs{MX: s.MX}).GetRecords(domain, ttl, origin)
		if err != nil {
	...
	if s.SPF != nil {
		spf_rrs, err := s.SPF.GetRecords(domain, ttl, origin)
		if err != nil {
	...
	if s.DMARC != nil {
		dmarc_rrs, err := s.DMARC.GetRecords(domain, ttl, origin)
		if err != nil {
	...
	if s.MTA_STS != nil {
		mta_sts_rrs, err := s.MTA_STS.GetRecords(domain, ttl, origin)
		if err != nil {
	...
	if s.TLS_RPT != nil {
		tls_rpt_rrs, err := s.TLS_RPT.GetRecords(domain, ttl, origin)
		if err != nil {
